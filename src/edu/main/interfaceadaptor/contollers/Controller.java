...
public class Controller {
    public static void control(String s, String[] args) {
        ...
        switch (s) {
            ...
            case "6" -> CommentEditer.editComment((CommentableUser) Login.loggedInUser, args[0], args[1], Integer.parseInt(args[2]));
            case "7" -> {
                if (Login.loggedInUser instanceof Professor) {
                    CommentDeleter.deleteComment((Professor) Login.loggedInUser, args[0], Integer.parseInt(args[1]));
                } else {
                    CommentDeleter.deleteComment((CommentableUser) Login.loggedInUser, args[0], Integer.parseInt(args[1]));
                }
